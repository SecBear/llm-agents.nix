...
let
  ...
  darwinDrv =
    let
      darwinAssets =
        hashes.${currentSystem} or (throw "Unsupported Darwin system for omp: ${currentSystem}");
      ompBinary = fetchurl {
        url = "https://github.com/can1357/oh-my-pi/releases/download/v${version}/${darwinAssets.binary.name}";
        hash = darwinAssets.binary.hash;
      };
      nativeAssets = map (
        asset:
        asset
    ...
    stdenvNoCC.mkDerivation {
      ...
      installPhase = ''
                ...
                cp ${ompBinary} $out/lib/omp/omp
                chmod +x $out/lib/omp/omp

        ${lib.concatMapStringsSep "\n" (
          asset: "        cp ${asset.src} $out/lib/omp/${asset.name}"
        ) nativeAssets}
      ...
      meta = with lib; {
        description = "A terminal-based coding agent with multi-model support (Darwin release binary)";
  ...
  linuxDrv = stdenv.mkDerivation {
    ...
    buildPhase =
      let
        platformTag =
          if stdenv.hostPlatform.isx86_64 then
            ...
            throw "Unsupported Linux platform for omp: ${currentSystem}";
        bunTarget =
          if stdenv.hostPlatform.isx86_64 then
      ...
      ''
        ...
        # Generate the napi type definitions and JS loader by running the napi
        # CLI from node_modules.
...
in
if stdenv.hostPlatform.isDarwin then darwinDrv else linuxDrv
